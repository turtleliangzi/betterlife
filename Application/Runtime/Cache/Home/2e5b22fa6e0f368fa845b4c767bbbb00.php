...
<div id="bl-mypage-content">
  ...
  <div class="bl-mypage-middle">
    ...
    <?php if($title == '编辑随笔' AND $article == ''): ?><div style="width:100%;text-align:center">
      <p style="line-height:30px;font-size:24px;">该文章不存在～～</p>
    </div>
    <?php else: ?>
    <div class="form-group" style="margin-top:10px">
      <label for="briefsummary-title"> 标题</label>
      <input type="text" value="<?php echo ($article['title']); ?>" class="form-control input-field" name="title" id="briefsummary-title" placeholder="请填写随笔标题">
    ...
    <div class="form-group">
      ...
      <select class="form-control" name="category">
        ...
        <?php if(is_array($categoryList)): foreach($categoryList as $key=>$category): if($article['categorys']['categoryid'] == $category['categoryid']): ?><option class="category" value="<?php echo ($category['categoryid']); ?>" selected><?php echo ($category['cname']); ?></option>
        <?php else: ?>
    ...
    <div class=form-group" style="text-align:center;margin-top:20px;margin-bottom:10px;">
      <?php if($title == '编辑随笔'): ?><button class="btn btn-success" id="edit-essay">编辑随笔</button>
      <?php else: ?>

      <button class="btn btn-success" id="add-essay">添加随笔</button><?php endif; ?>
    </div><?php endif; ?>
  </div>
</div>
