...
<div id="bl-mypage-content">
  ...
  <div class="bl-mypage-middle">
    ...
    <?php if(empty($essayList)): ?><div style="width:100%;text-align:center">
    ...
    <table class="table table-hover">
      ...
      <tbody>
        <?php if(is_array($essayList)): foreach($essayList as $k=>$essay): ?><tr>
          <td><?php echo ($k+1); ?></td>
          <td><a href="/article/myarticle?articleid=<?php echo ($essay['conclusionid']); ?>&type=<?php echo ($essay['type']); ?>"><?php echo ($essay['title']); ?></a></td>
          <td><?php echo ($essay['categorys']['cname']); ?></td>
          <td><?php if(is_array($essay['tag'])): $i = 0; $__LIST__ = $essay['tag'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$tag): $mod = ($i % 2 );++$i; echo ($tag['signname']); ?>、<?php endforeach; endif; else: echo "" ;endif; ?></td>
          <td><?php echo (date("Y-m-d H:m:s", $essay['createtime'] )); ?></td>
          <td><?php if($essay['share'] == 1): ?>是<?php else: ?>否<?php endif; ?></td>
          <td><a href="/article/addessay?articleid=<?php echo ($essay['conclusionid']); ?>&type=<?php echo ($essay['type']); ?>">编辑</a></td>
