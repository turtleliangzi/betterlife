...
<div id="bl-mypage-content">
  ...
  <div class="bl-mypage-middle">
    <h4>我的资源&nbsp;&nbsp; <small><i style="color:#669966" class="fa fa-folder-open"></i><?php echo ($folder['foldername']); ?></small></h4>
    <?php if(empty($sourceList)): ?><div style="width:100%;text-align:center">
    ...
    <table class="table table-hover">
      ...
      <tbody>
        <?php if(is_array($sourceList)): foreach($sourceList as $k=>$source): ?><tr>
          <td><?php echo ($k+1); ?></td>
          <td><a href="/source/source?rid=<?php echo ($source['rid']); ?>"><img class="img-circle" src="/Uploads/<?php echo ($source['thumb']); ?>" style="height:20px;width:20px;">&nbsp;&nbsp;<?php echo ($source['rname']); ?></a></td>
          <td><?php echo ($source['storeplace']); ?></td>
          <td><?php echo ($source['category']['cname']); ?></td>
          <td><?php if(is_array($source['tag'])): $i = 0; $__LIST__ = $source['tag'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$tag): $mod = ($i % 2 );++$i; echo ($tag['signname']); ?>、<?php endforeach; endif; else: echo "" ;endif; ?></td>
          <td><?php echo (date("Y-m-d", $source['createtime'] )); ?></td>
          <td><a href="/source/addsource?rid=<?php echo ($source['rid']); ?>">编辑</a></td><?php endforeach; endif; ?>
      </tbody>
    </table><?php endif; ?>
  </div>
</div>
