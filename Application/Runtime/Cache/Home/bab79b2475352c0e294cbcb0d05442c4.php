...
<div id="bl-mypage-content">
  ...
  <div class="bl-mypage-middle">
    ...
    <?php if(empty($summaryList)): ?><div style="width:100%;text-align:center">
    ...
    <table class="table table-hover">
      ...
      <tbody>
        <?php if(is_array($summaryList)): foreach($summaryList as $k=>$summary): ?><tr>
          <td><?php echo ($k+1); ?></td>
          <td><a href="/article/myarticle?articleid=<?php echo ($summary['conclusionid']); ?>&type=<?php echo ($summary['type']); ?>"><?php echo ($summary['title']); ?></a></td>
          <td><?php echo ($summary['categorys']['cname']); ?></td>
          <td><?php if(is_array($summary['tag'])): $i = 0; $__LIST__ = $summary['tag'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$tag): $mod = ($i % 2 );++$i; echo ($tag['signname']); ?>、<?php endforeach; endif; else: echo "" ;endif; ?></td>
          <td><?php echo (date("Y-m-d H:m:s", $summary['createtime'] )); ?></td>
          <td><?php if($summary['share'] == 1): ?>是<?php else: ?>否<?php endif; ?></td>
          <td><a href="/article/addsummary?articleid=<?php echo ($summary['conclusionid']); ?>&type=<?php echo ($summary['type']); ?>">编辑</a></td>
